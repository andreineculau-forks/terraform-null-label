...
| <a name="input_label_order_dns"></a> [label\_order\_dns](#input\_label\_order\_dns) | Labels to concatenate with `.` and append after the delimiter-joined portion of `id`.<br/>Labels listed here are removed from `label_order` when constructing `id`.<br/>Defaults to `["namespace"]` for `google_storage_bucket_dns` and `[]` for other styles.<br/>Valid labels are `namespace`, `tenant`, `environment`, `stage`, `name`, and `attributes`. | `list(string)` | `null` | no |
